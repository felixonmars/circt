...
void ConstructLECPass::runOnOperation() {
  ...
  if (moduleA.getModuleType() != moduleB.getModuleType()) {
    moduleA.emitError("module's IO types don't match second module's: ")
  ...
  auto *yieldA = moduleA.getBodyBlock()->getTerminator();
  auto *yieldB = moduleB.getBodyBlock()->getTerminator();
  ...
  {
    OpBuilder::InsertionGuard guard(builder);
    builder.setInsertionPoint(yieldA);
    builder.create<verif::YieldOp>(loc, yieldA->getOperands());
    yieldA->erase();
    builder.setInsertionPoint(yieldB);
    builder.create<verif::YieldOp>(loc, yieldB->getOperands());
    yieldB->erase();
  }
